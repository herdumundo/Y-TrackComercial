    ...
    Scaffold(
        ...
        bottomBar = {
            ...
            BottomAppBar(
                ...
                content = { MyBottomBar( prepareBottomMenu(context,visitaSupervisorViewModel))
                })
...
fun MyApp(
    ...
    val cuadroLoading by visitaSupervisorViewModel.cuadroLoading.observeAsState(true)
    val cuadroLoadingMensaje by visitaSupervisorViewModel.cuadroLoadingMensaje.observeAsState("")
...
private fun prepareBottomMenu(
    ...
    // add menu items
    bottomMenuItemsList.add( BottomMenuItem(label = "OBTENER PERMISOS",icon = Icons.Filled.PunchClock,{visitaSupervisorViewModel.obtenerPermisos() }))
    bottomMenuItemsList.add( BottomMenuItem(label = "OBTENER UBICACIÓN",icon = Icons.Filled.TrackChanges) {visitaSupervisorViewModel.abrirmapaGoogleMaps(
        context
    )
    })
