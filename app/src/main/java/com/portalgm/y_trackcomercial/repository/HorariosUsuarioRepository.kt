...
import com.portalgm.y_trackcomercial.util.HoraActualUtils.convertirStringToLocalTime
import com.portalgm.y_trackcomercial.util.SharedPreferences
import com.portalgm.y_trackcomercial.util.ValidacionesVisitas.ValidacionInicioHoraResult
...
class HorariosUsuarioRepository @Inject constructor(
    ...
    private val visitasDao: com.portalgm.y_trackcomercial.data.model.dao.registroDaos.VisitasDao,
    private val sharedPreferences: SharedPreferences,
    private val horariosUsuarioDao: com.portalgm.y_trackcomercial.data.model.dao.HorariosUsuarioDao
) {
    ...
    private fun convertToEntityList(horariosResponseList: List<com.portalgm.y_trackcomercial.data.model.models.HorariosUsuarioResponse>): List<com.portalgm.y_trackcomercial.data.model.entities.HorariosUsuarioEntity> {
        val entityList = mutableListOf<com.portalgm.y_trackcomercial.data.model.entities.HorariosUsuarioEntity>()
        for (horarioResponse in horariosResponseList) {
            for (yemsysHorario in horarioResponse.yemsys_horarios) {
                for (yemsysTurno in yemsysHorario.yemsys_turnos) {
                    val entity =
                        com.portalgm.y_trackcomercial.data.model.entities.HorariosUsuarioEntity(
                            id = yemsysTurno.id,
    ...
    suspend fun validacionInicioHora(esPrimeraVisita: Boolean): ValidacionInicioHoraResult {
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (horarioActualList.isEmpty()&&esPrimeraVisita) {
                ...
                respuestaVisita = 4
                mensaje = "NO EXISTEN TURNOS PARA LA HORA ACTUAL."
            }
            ...
            else if(horarioActualList.isEmpty())
            {
                ...
                if (esPrimeraVisita) {
                    ...
                    if (horaEntradaMas5Minutos.isBefore(horaActualLocalTime)) {
                        respuestaVisita = 2
                        mensaje = "LLEGADA TARDIA, LA TOLERANCIA ES HASTA LAS $horaEntradaMas5Minutos "
                    }
